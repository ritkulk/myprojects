...
params = {'batch_size': 100,
          'sent_maxlen': sent_maxlen,  # maximum words in a sentence
...
for n_loop in range(n_folds):
    split_ratio = 0.8  # fraction of data, set aside for train, rest for test
    ...
    # iteration steps for keras generator
...
print('Mean Accuracy:{},Mean Precision:{},Mean Recall:{},Mean Fscore:{}'.format(
         numpy.mean(results[:, 0]), numpy.mean(results[:, 1]),
         numpy.mean(results[:, 2]), numpy.mean(results[:, 3])))
